...
def connect():
    conn = sqlite.connect('jukebox/static/config/jukebox.db')
...
def get_all_albums(artist):
    ...
    albums = cursor.execute('SELECT artist.id artist_id, artist.name artist_name, album.* \
                            FROM album, artist WHERE artist.id=album.artist_id AND artist.name="%s" \
                            ORDER BY album.year' % artist).fetchall()

    for album in albums:
        musics = get_all_musics(album['id'])
        album['musics'] = musics

    conn.close()
    return albums

def get_all_musics(album):
    conn, cursor = connect()
    musics = cursor.execute('SELECT artist.id artist_id, artist.name artist_name, album.id album_id, album.name album_name, \
                            ...
                            ORDER BY music.track' % album).fetchall()
    conn.close()
    return musics

## QUERYs START WITH
def get_artist_starts_with(letter):
...
def get_total_info_starts_with(letter):
## FIXME = Selecionado o # retorna 0
    conn, cursor = connect()
    artists = cursor.execute('SELECT COUNT(*) FROM artist WHERE upper(name) LIKE "%s%%"' % letter).fetchone().values()[0]
    albums  = cursor.execute('SELECT COUNT(*) FROM artist, album WHERE artist.id = album.artist_id and upper(artist.name) LIKE "%s%%"' % letter).fetchone().values()[0]
    musics  = cursor.execute('SELECT COUNT(*) FROM artist, album, music WHERE artist.id = album.artist_id and album.id = music.album_id and upper(artist.name) LIKE "%s%%"' % letter).fetchone().values()[0]
    conn.close()
    return artists, albums, musics

def get_total_info_of_artist(artist):
## FIXME = Selecionado o # retorna 0
    conn, cursor = connect()
    albums  = cursor.execute('SELECT COUNT(*) FROM artist, album WHERE artist.id = album.artist_id and upper(artist.name) LIKE "%s"' % artist).fetchone().values()[0]
    musics  = cursor.execute('SELECT COUNT(*) FROM artist, album, music WHERE artist.id = album.artist_id and album.id = music.album_id and upper(artist.name) LIKE "%s"' % artist).fetchone().values()[0]
    conn.close()
    return albums, musics

## QUERYs GET TOTAL
def get_total_info():
    conn, cursor = connect()
    artists = cursor.execute('SELECT COUNT(*) FROM artist').fetchone().values()[0]
    albums  = cursor.execute('SELECT COUNT(*) FROM album').fetchone().values()[0]
    musics  = cursor.execute('SELECT COUNT(*) FROM music').fetchone().values()[0]
    conn.close()
    return artists, albums, musics

def get_total_size():
    conn, cursor = connect()
    total_size = cursor.execute('SELECT SUM(size) from music').fetchone().values()[0]
    conn.close()
    return total_size

def get_total_time():
    conn, cursor = connect()
    total_size = cursor.execute('SELECT SUM(length) from music').fetchone().values()[0]
    conn.close()
    return total_size
